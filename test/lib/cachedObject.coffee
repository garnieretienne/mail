...
describe 'Cached Objects >', ->
  ...
  describe 'Pure Cached Object >', ->
    ...
    it 'should find a cached object using any cached attribute (testing WHERE)', (done) ->
      object = new CachedObject()
      object.name = 'Amazing Cached Object'
      object.goal = 'Manage cache for object attributes'
      object.cachedAttributes = ['name', 'goal']
      object.save ->
        CachedObject.find {where: {name: 'Amazing Cached Object', goal: 'Manage cache for object attributes'}}, (err, cachedObjects) ->
          throw err if err
          expect(cachedObjects[0].name).to.equal object.name
          expect(cachedObjects[0].goal).to.equal object.goal
          done()

    it 'should find many object (testing WHERE and LIMIT)', (done) ->
      object1 = new CachedObject()
      object1.name = 'Object1'
      object1.goal = 'Testing things'
      object2 = new CachedObject()
      object2.name = 'Object2'
      object2.goal = 'Testing things'
      object1.cachedAttributes = ['name', 'goal']
      object2.cachedAttributes = ['name', 'goal']
      object1.save (err) ->
        throw err if err
        object2.save (err) ->
          throw err if err
          CachedObject.find {limit: 2}, (err, cachedObjects) ->
            expect(cachedObjects.length).to.equal 2
            CachedObject.find {where: {goal: 'Testing things'}, limit: 1}, (err, cachedObjects2) ->
